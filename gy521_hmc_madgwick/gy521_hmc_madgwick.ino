...
int16_t magOffsets[3] = { 130, -126, 213 }; 
...
void loop()
{
  ...
  if (Serial.available() > 0)
  {
    ...
    {
      t.every(10,ler_sensor_inercial); //chama a cada 10ms = 1000/20
      for (int i = 0; i < 100; i++) {
        delay(10);
      }
...
void ler_sensor_inercial()
{
  ...
  //Serial.print(String(deg[0]) + " " + String(deg[1]) + " " + String(deg[2]) + " ");  
  //Serial.print(String(mx) + " " + String(my) + " " + String(mz) + "\n");  
  enviar_pacote_inercial(quat);  
}

void enviar_pacote_inercial(float* _quat)
{
  uint16_t w = float2uint16(_quat[0]);
  uint16_t x = float2uint16(_quat[1]);
  uint16_t y = float2uint16(_quat[2]);
  uint16_t z = float2uint16(_quat[3]);
  //Assembling packet and sending
  serial_buffer_out[0] = UART_START;
  serial_buffer_out[1] = UART_PQUAT;
  serial_buffer_out[2] = w>>8; //qw_msb
  serial_buffer_out[3] = w&&0xFF; //qw_lsb
  serial_buffer_out[4] = x>>8; //qx_msb
  serial_buffer_out[5] = x&&0xFF; //qx_lsb
  serial_buffer_out[6] = y>>8; //qy_msb
  serial_buffer_out[7] = y&&0xFF; //qy_lsb
  serial_buffer_out[8] = z>>8; //qz_msb
  serial_buffer_out[9] = z&&0xFF; //qz_lsb
